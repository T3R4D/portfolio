...
for j =1:L
    Winc = sum(dW(R*(j-1)+1:R*j));
    Ntemp = Ntemp + Dt*r*Ntemp*(1-Ntemp/K)+epsilon*Ntemp*(1-Ntemp/K)*Winc;
    Nem(j) = Ntemp;
end

plot([0:Dt:T],[Nzero,Nem],'m');
hold on 
...
plot([0:Dt:T],[Nzero,Nem],'g'); 
legend('\epsilon = 0.1 Eq 1', '\epsilon = 0.3 Eq 1', '\epsilon = 0.1 Eq 2', '\epsilon = 0.3 Eq 2')
hold off
